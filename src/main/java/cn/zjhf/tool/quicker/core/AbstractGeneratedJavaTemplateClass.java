...
import cn.zjhf.tool.quicker.dto.PackageInfo;
...
public abstract class AbstractGeneratedJavaTemplateClass extends AbstractGeneratedJavaClass {
    ...
    public AbstractGeneratedJavaTemplateClass(GeneratorParam generatorParam, TableInfo tableInfo) {
        ...
        // 查询参数类
        String queryDtoClassName = String.format("%s.%s.%sQueryDTO", basePackage, packageInfo.getDtoPackage(), modelName);
        queryDtoClassName = super.parseJavaImportType(queryDtoClassName);
        model.setQueryDtoClassName(queryDtoClassName);
        String queryDtoObjectName = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_CAMEL, queryDtoClassName);
        model.setQueryDtoObjectName(queryDtoObjectName);

        // 查询参数类
        String pageQueryDtoClassName = String.format("%s.%s.%sPageQueryDTO", basePackage, packageInfo.getDtoPackage(), modelName);
        pageQueryDtoClassName = super.parseJavaImportType(pageQueryDtoClassName);
        model.setPageQueryDtoClassName(pageQueryDtoClassName);
        String pageQueryDtoObjectName = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_CAMEL, pageQueryDtoClassName);
        model.setPageQueryDtoObjectName(pageQueryDtoObjectName);

        // "/mapping"
        String classMapping = modelObjectName;
        model.setClassMapping(classMapping.toLowerCase());
